...
// GET /api/file-areas — all areas + points — public, read-only
router.get('/', async (req, res) => {
    try {
        ...
        console.error('[FILE-AREAS] get error:', err);
        res.status(500).json({ error: err.message });
    }
});

// GET /api/file-areas/:id/incidents — incidents currently intersecting this area's
// polygon — public, computed on-demand (not cached, unlike getIncidentCounts()).
router.get('/:id/incidents', async (req, res) => {
    try {
        const incidents = await getMatchingIncidents(req.params.id);
        res.json(incidents);
    } catch (err) {
        console.error('[FILE-AREAS] incidents error:', err);
        res.status(500).json({ error: err.message });
    }
});

// POST /api/file-areas — create area with points
router.post('/', auth, async (req, res) => {
    ...
    try {
        ...
        const [result] = await pool.query(`
            INSERT INTO file_areas (name, description, color, minLat, maxLat, minLng, maxLng)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        ...
        console.error('[FILE-AREAS] post error:', err);
        res.status(500).json({ error: err.message });
    }
});
...
router.put('/:id', auth, async (req, res) => {
    ...
    try {
        ...
        console.error('[FILE-AREAS] put error:', err);
        res.status(500).json({ error: err.message });
    }
});

// PUT /api/file-areas/:id/points — replace all points and recalculate bbox
router.put('/:id/points', auth, async (req, res) => {
    ...
    try {
        const bbox = calcBbox(points);
        await pool.query(`DELETE FROM file_area_points WHERE areaId = ?`, [req.params.id]);
        await insertPoints(req.params.id, points);
        await pool.query(`
            UPDATE file_areas SET minLat = ?, maxLat = ?, minLng = ?, maxLng = ? WHERE id = ?
        `, [bbox.minLat, bbox.maxLat, bbox.minLng, bbox.maxLng, req.params.id]);
        const area = await getAreaById(req.params.id);
        res.json({ updatedRecord: area });
    } catch (err) {
        console.error('[FILE-AREAS] put points error:', err);
        res.status(500).json({ error: err.message });
    }
});

// DELETE /api/file-areas/:id
router.delete('/:id', auth, async (req, res) => {
    try {
        await pool.query(`DELETE FROM file_areas WHERE id = ?`, [req.params.id]);
        res.json({ success: true });
    } catch (err) {
        console.error('[FILE-AREAS] delete error:', err);
        res.status(500).json({ error: err.message });
    }
});
...
async function insertPoints(areaId, points) {
    ...
    await pool.query(`INSERT INTO file_area_points (areaId, latitude, longitude, orderIndex) VALUES ?`, [values]);
}

async function getAreaById(id) {
    const [[area]] = await pool.query(`SELECT * FROM file_areas WHERE id = ?`, [id]);
    const [points] = await pool.query(`
        SELECT latitude AS lat, longitude AS lng
        FROM file_area_points WHERE areaId = ? ORDER BY orderIndex
    `, [id]);
    return { ...area, points };
}
