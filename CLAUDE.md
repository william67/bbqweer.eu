...
- Bliksem (`/lightning`) — real-time lightning strike map via Blitzortung/lightningmaps.org WebSocket; strikes stored in Redis (10min TTL); Socket.IO pushes live updates; yellow bolt = active (<60s), grey = older; thunder ring animates at zoom ≥ 11; counter shows viewport/active counts
