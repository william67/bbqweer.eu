...
async function processFile(fileRow, fullSync = false) {
    ...
    try {
        ...
        if (!entry) { await logMsg(`ERROR: ${filename} not found in ZIP`); await taskError('knmidata-sync'); return true; }
    ...
    if (!station) { await logMsg(`ERROR: could not determine station for ${filename}`); await taskError('knmidata-sync'); return true; }
    ...
    try {
        ...
        else { await logMsg(`Unknown FILETYPE '${filetype}' for ${filename}`); await taskError('knmidata-sync'); return true; }
    } catch (e) {
...
async function syncKnmiDataV3(fullSync = false) {
    ...
    try {
        ...
        let progress = 0, errors = 0;
        for (const fileRow of files) {
            const hadError = await processFile(fileRow, fullSync);
            if (hadError) errors++;
        ...
        await logMsg('KNMI data sync v3 finished.');
        const status = errors > 0 ? 'partial' : 'success';
        await taskFinish('knmidata-sync', status, `Files: ${files.length}, Errors: ${errors}${fullSync ? ' (full sync)' : ''}`);
    } catch (e) {
