...
│   ├── lightning-map.md
│   ├── ntfy-server.md      — self-hosted ntfy push notification server (shared with wo-ict.nl, see "Shared across projects")
...
- Taakstatus dialog — in login dropdown, polls `/api/server-tasks` every 2s while open (logged-in only). Any one-shot task failure (`taskFinish(taskCode, 'error', message)`) auto-pushes to the shared `servererrors` ntfy topic (see `docs/ntfy-server.md` "Task/server-error alerts"); its header row has a **"Test bericht" button** (next to the title, via a `pTemplate="header"` override — `POST /api/ntfy/test`, `{type: 'server'}`) to confirm delivery on demand.
